...
for i = 1:4
    if(safe(i, 5-row(1)) == 1)
        safe_location.x = i;
        safe_location.y = 5-row(1);
        if(safe_location.x < 5-row(1))
            safe_location.dir = 0;
        else
            safe_location.dir = 2;
        end
...
[solution, nodes] = CS4300_Wumpus_A_star(board, [agent.x, agent.y, agent.r]...
, [safe_location.x, safe_location.y, 0], 'CS4300_A_star_Man');
...
end_state = nodes(end).state;
dif = abs(end_state.dir - safe_location.dir);

if(dif == 1)
   if(end_state.dir < safe_location.dir)
      plan(end+1) = 3;
   else
      plan(end+1) = 2;
   end
